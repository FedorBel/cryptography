...
int main()
{
	...
	string find = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
	vector<int> find_int; // Сравниваем с этими символами

	vector<int> ct;
	vector<vector<int> > cti(11);
	vector<int> target_text;



	equal(cts); // Сделать все строки одной длины

	getInt(cts, cti); // Перевести шифр из строки в число
	getInt(find, find_int);

	vector<int> key_guess; // Ключ от 0 до 256
	for (int i = 0; i < 256; i++)
	{
		key_guess.push_back(i);
	}
	vector<vector<int> > num_f(256); // Частота появления числа
	for (int i = 0; i < 256; i++)
	{
		for (int j = 0; j < 83; j++)
		{
	...
	xOr(cti, key_guess, find_int, num_f);

	vector<int> key(cti[1].size());

	max_f(num_f, key);

	xOr(cti, key, target_text);

	clear_file();
	file_write(target_text);
